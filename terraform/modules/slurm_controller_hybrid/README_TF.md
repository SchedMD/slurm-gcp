...
| [google_pubsub_subscription_iam_member.compute_pull_subscription_sa_binding_subscriber](https://registry.terraform.io/providers/hashicorp/google/latest/docs/resources/pubsub_subscription_iam_member) | resource |
...
| <a name="input_enable_bigquery_load"></a> [enable\_bigquery\_load](#input\_enable\_bigquery\_load) | Enables loading of cluster job usage into big query.<br><br>NOTE: Requires Google Bigquery API and 'roles/bigquery.dataEditor' on the<br>controller service account. | `bool` | `false` | no |
...
| <a name="input_enable_reconfigure"></a> [enable\_reconfigure](#input\_enable\_reconfigure) | Enables automatic Slurm reconfigure on cluster delta (e.g. partition).<br>Configuration files will be regenerated and propagated.<br><br>NOTE: Requires Google PubSub API. | `bool` | `true` | no |
