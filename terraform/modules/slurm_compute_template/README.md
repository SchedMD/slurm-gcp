# Module: Slurm Compute Template

This module creates an instance template for a Slurm cluster compute node.

## Usage

See the [simple](../../examples/slurm_compute_template/simple) example for a
usage example.
...
| <a name="input_service_account"></a> [service\_account](#input\_service\_account) | Service account to attach to the instances.<br>* email : Service account email.<br>* scopes : Set of service account scopes. | <pre>object({<br>    email  = string<br>    scopes = set(string)<br>  })</pre> | <pre>{<br>  "email": "default",<br>  "scopes": [<br>    "https://www.googleapis.com/auth/cloud-platform"<br>  ]<br>}</pre> | no |
...
| <a name="input_source_image_family"></a> [source\_image\_family](#input\_source\_image\_family) | Source image family. | `string` | `"schedmd-slurm-21-08-2-hpc-centos-7"` | no |
| <a name="input_source_image_project"></a> [source\_image\_project](#input\_source\_image\_project) | Project where the source image comes from. If it is not provided, the provider project is used. | `string` | `"schedmd-slurm-public"` | no |
