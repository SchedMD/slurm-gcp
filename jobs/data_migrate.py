...
def dict_to_kv(dict):
    ...
    return ','.join(f"{k}={','.join(v) if isinstance(v, list) else v}" for k, v in dict.items() if v is not None)


def run(args, stdout=subprocess.PIPE, stderr=subprocess.PIPE, shell=False,
