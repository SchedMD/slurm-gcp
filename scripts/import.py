...
def main():
    ...
    compute_list = run(f"gcloud compute instances list --uri --filter='tags.items={cluster_name} AND tags.items=\"compute\"'", shell=True, check=True, get_stdout=True).stdout.strip().split('\n')
